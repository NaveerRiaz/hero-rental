...
const Home = () => {
    ...
    const removePriceFilter = () => {
        const filteredObj = Object.fromEntries(
            Object.entries(filters).filter(([key]) => !["price_min", "price_max"].includes(key))
        );
    ...
    const toggleBrandFilter = (brandName) => {
        setFilters((prev) => {
            ...
            if (updatedBrands.includes(brandName)) {
                return { ...prev, brands: updatedBrands.filter((item) => item !== brandName) };
            } else {
    ...
    useEffect(() => {
        getData(db).then((data) => {
            ...
            let filteredData = vehicleType ? data.filter((item) => item.Car_status && item.Car_status.toLowerCase() === vehicleType) : data;

            // filter for price
            if (filters.price_min) {
                const min_price = filters.price_min;
                const max_price = filters.price_max;
                filteredData = filteredData.filter((item) => item.Car_price && item.Car_price >= min_price && item.Car_price <= max_price);
            }

            // filter for brands
            if (filters.brands && filters.brands.length > 0) {
                filteredData = filteredData.filter((item) => filters.brands.includes(item.Car_brand.toLowerCase()));
            }
    ...
    return (
        <>
            ...
            {showFilters ? (
                <>
                    <div className="fixed top-0 w-screen h-screen bg-black opacity-30 z-50">
                    </div>
                    <div onClick={() => setShowFilters(false)} className="top-0 h-full z-50 fixed w-full">
                        <div onClick={(e) => e.stopPropagation()} className="mx-auto py-10 flex flex-col items-center h-full justify-between bg-white rounded-lg w-[35vw]">
                            <div className='w-full px-4'>
                                ...
                                <div className='gap-1 flex flex-col'>
                                    ...
                                    <div className="w-full flex justify-between h-[40px]">
                                        <div className="flex bg-gray-200 px-2 py-1 rounded-lg items-center">
                                            <input id='price-min' placeholder='From' type="text" className='bg-inherit focus:outline-none' />
                                            <p className="">AED</p>
                                        </div>

                                        <div className="flex bg-gray-200 px-2 py-1 rounded-lg items-center">
                                            <input id='price-max' placeholder='To' type="text" className='bg-inherit focus:outline-none' />
                                            <p className="">AED</p>
                                        </div>
                                ...
                                <ul className='text-lg '>
                                    <div className="flex gap-2 hover:text-red-300 hover:cursor-pointer">
                                        <input type='checkbox' onChange={() => selectCarType("luxury")} checked={vehicleType === "luxury"} className=''></input>
                                        <p className="">Luxury</p>
                                    </div>
                                    <div className="flex gap-2 hover:text-red-300 hover:cursor-pointer">
                                        <input type='checkbox' onChange={() => selectCarType("economy")} checked={vehicleType === "economy"} className=''></input>
                                ...
                                <ul className='text-lg '>
                                    ...
                                    {Object.entries(brands).map(([brandName, numberOfVehc]) => (
                                        <div key={brandName} className="flex gap-2 hover:text-red-500 hover:cursor-pointer">
                                            <input type='checkbox' className='' onChange={() => toggleBrandFilter(brandName)} checked={filters.brands && filters.brands.includes(brandName)}></input>
                                            <p className="">{`${brandName[0].toUpperCase()}${brandName.substring(1)} (${numberOfVehc})`}</p>
                                        </div>
                                    ))}
                            ...
                            <button onClick={applyFilters} className="text-lg font-medium bg-red-200 text-black outline outline-red-500 w-[80%] py-2 rounded-lg">Apply Filters</button>
                        </div>
                    </div>
                </>) : (<></>)}
            ...
            <div className='relative w-full h-screen'>
                ...
                {/* navigation bar */}
                <div className={`md:flex hidden justify-between py-8 px-16 items-center top-0 fixed w-full z-10 ${scroll > 0 ? "bg-white shadow-xl" : "bg-none"}`}>
                ...
                <img className='w-full top-0 absolute -z-10 opacity h-screen object-cover' src={HeroImage} alt="" />

                <div className="absolute top-[10%] mx-auto w-full flex items-center justify-center">
                    <span className="text-6xl font-medium text-black">
                        Find Your
                    </span>
                    <span className='text-red-700 text-6xl font-bold mx-3'>Perfect Ride</span>
                    <span className="text-6xl font-medium text-black">
                        in Dubai
                    </span>
                </div>
            ...
            <div className="flex flex-col gap-12 w-full my-10">
                <h1 className="text-5xl text-center w-full font-medium">
            ...
            <div className="flex flex-col gap-12 w-full my-20">
                ...
                <div className="flex gap-12 items-center justify-center">
                    <div onClick={() => selectCarType("luxury")}
                        className={
                            vehicleType === "luxury"
                    ...
                    <div onClick={() => selectCarType("economy")}
                        className={
                            vehicleType === "economy"
            ...
            <div id='vehicles' className="my-20 w-full px-12">
                ...
                <div className="flex gap-4">
                    <button onClick={() => { setShowFilters(true) }} className='bg-black text-white px-4 rounded-xl text-lg font-medium outline outline-red-500 hover:text-black hover:bg-red-500 py-1'>
                        Filters
                    </button>

                    {filters.price_min ? (<>
                        <div onClick={removePriceFilter} className="px-4 py-2 flex gap-4 outline outline-1 rounded-xl hover:bg-red-200 hover:cursor-pointer">
                            <p>AED {filters.price_min}-{filters.price_max}</p>
                            <img src="images/xmark-solid.svg" width={15} alt="" />
                        </div>
                    </>) : (<></>)}


                    {filters.brands ? filters.brands.map((brandName, index) =>
                        <div key={index} onClick={() => toggleBrandFilter(brandName)} className="px-4 py-2 flex gap-4 outline outline-1 rounded-xl hover:bg-red-200 hover:cursor-pointer">
                            <p>{`${brandName[0].toUpperCase()}${brandName.substring(1)}`}</p>
                            <img src="images/xmark-solid.svg" width={15} alt="" />
                        </div>
                    ) : (<></>)}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 w-full my-20 gap-12">
                    ...
                    {carsData ? Array.from(carsData).map((item, index) => (

                        <div onClick={()=>navigate(`/details/${item.id}`)} key={index} className="flex flex-col col-span-1 rounded-3xl bg-white shadow-xl gap-1 pb-4">
                            <div className="w-full h-[300px]">
                                <img src={item.Car_image ? item.Car_image[0] : "https://placehold.co/600x400?text=No-Image"} className='w-full h-full object-cover rounded-xl' alt="" />
                            </div>
                            <div className="py-4 px-6">
                                <p className="text-3xl font-medium w-full">{item.Car_name}</p>
                                <p className="text-2xl w-full">{item.Car_price ? `AED ${item.Car_price}` : "Call to confirm"}</p>
                                <p className="text-2xl text-gray-500">{item.Car_seats ? `${item.Car_seats.split(" ")[0]} Doors` : ""} </p>
                            </div>
                        </div>
                        ...
                        Array(6).fill(0).map((item, index) => (
                            <div key={index} className="animate-pulse justify-center flex flex-col col-span-1 rounded-3xl p-8 bg-white shadow-xl gap-2">
                                <div className="w-full h-[300px">
                                    <img src="https://placehold.co/600x400?text=Loading" className='w-full h-full object-contain rounded-lg' alt="" />
                                </div>
